...
:class:`~oop_ml.numpy.preprocessing.standardization.standardizer.Standardizer` rather
than in
``regression``.
...
or
:class:`~oop_ml.numpy.regression.penalised.lasso_regression.LassoRegression` to keep the
extra freedom in check, and with
:class:`~oop_ml.numpy.preprocessing.standardization.standardizer.Standardizer` because
``x ** 9`` on raw units produces columns whose scales differ by orders of magnitude.
"""
