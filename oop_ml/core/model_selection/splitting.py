...
class Splits:
    ...
    ----------
    splits:
        One :class:`~oop_ml.core.model_selection.dataset.DataSplit` per partition.
...
class TrainTestSplitter(BaseModel):
    ...
    def split(self, dataset: Dataset) -> DataSplit:
        ...
        3. ``dataset.select_rows(...)`` on each slice, and pair them in a
           :class:`~oop_ml.core.model_selection.dataset.DataSplit`.
...
class KFold(BaseModel):
    ...
    def split(self, dataset: Dataset) -> Splits:
        """Produce one :class:`~oop_ml.core.model_selection.dataset.DataSplit` per fold.
        ...
        3. ``dataset.select_rows(...)`` on each, pair them in a
           :class:`~oop_ml.core.model_selection.dataset.DataSplit`, and collect the lot
           into
           :class:`Splits`.
